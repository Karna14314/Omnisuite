...
object ImpositionEngine {
    ...
    private fun calculateCardsLayout(
        ...
        when (config.cardMode) {
            CardMode.REPEAT -> {
                // Repeat single page across the whole grid for each page in document
            ...
            CardMode.SEQUENTIAL -> {
                // Continuous sequential grid layout across multiple sheets
                return calculateNUpLayout(pageCount, sourceWidthPt, sourceHeightPt, config)
            }

            CardMode.DUPLEX -> {
                // Duplex layout: Front sheet has grid left-to-right, Back sheet mirrors horizontally for back-to-back print alignment
                var currentPage = 0
    ...
    private fun calculateCropResizeLayout(
        ...
        // Compute source width/height after crop box
    ...
    private fun calculateBleedAndMarksLayout(
        ...
        val bleedPt = PaperSize.mmToPoints(config.bleedMm)
        val markMarginPt = PaperSize.mmToPoints(15f) // Extra padding around page for crop marks and registration targets
    ...
    private fun calculateZineLayout(
        ...
        return when (config.zineType) {
            ZineType.ONE_SHEET_8PAGE -> {
                // 1 sheet, 8-page mini-zine layout on A4 landscape (4 columns x 2 rows)
                // Top row (Pages 8, 7, 6, 5 rotated 180° upside down)
                // Bottom row (Pages 1, 2, 3, 4 upright)
                ...
                // Mini Zine standard order
                // Top row (Upside down - rotated 180°): Page 8, 7, 6, 5
                // Bottom row (Upright - 0°): Page 1, 2, 3, 4
                val pageOrderTop = listOf(7, 6, 5, 4) // 0-indexed page indices (8, 7, 6, 5)
                val pageOrderBottom = listOf(0, 1, 2, 3) // 0-indexed page indices (1, 2, 3, 4)
            ...
            ZineType.CUT_STACK_FOLD -> {
                // Stacked multi-sheet zine layout using booklet saddle-stitch logic
